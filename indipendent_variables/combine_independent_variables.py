...
# Define paths
base_path = "/Group-project-Data-Science-in-Space/indipendent_variables/"
...
press_clean = press_freedom[press_freedom["Year"] == latest_year][
    ["Country", "ISO", "Score"]
].copy()
...
factor_rows = rule_of_law_wide[
    rule_of_law_wide["Country"].str.contains("Factor", na=False)
]
...
factor1_idx = rule_of_law_wide[
    rule_of_law_wide["Country"] == "Factor 1: Limited Government Powers"
].index[0]
factor1_data = rule_of_law_wide.iloc[
    factor1_idx, 2:
].copy()  # Skip 'sheet_name' and 'Country' columns

# Create a dataframe with country names as rows
rule_of_law_clean = pd.DataFrame(
    {"Country": factor1_data.index, "Rule_of_Law_Score": factor1_data.values}
)

# Convert score to numeric (handle any non-numeric values)
rule_of_law_clean["Rule_of_Law_Score"] = pd.to_numeric(
    rule_of_law_clean["Rule_of_Law_Score"], errors="coerce"
)
# Convert from 0-1 scale to 0-100 scale for consistency
rule_of_law_clean["Rule_of_Law_Score_Pct"] = (
    rule_of_law_clean["Rule_of_Law_Score"] * 100
)
...
iso3_mapping = pd.concat(
    [corruption_clean[["Country", "ISO3"]], press_clean[["Country", "ISO3"]]]
...
combined = combined.merge(
    press_clean[["ISO3", "Press_Freedom_Score"]], on="ISO3", how="outer"
)

# Merge literacy
combined = combined.merge(
    literacy_clean[["ISO3", "Literacy_Rate_Pct"]], on="ISO3", how="outer"
)

# Merge rule of law
combined = combined.merge(
    rule_of_law_clean[["ISO3", "Rule_of_Law_Score_Pct"]], on="ISO3", how="outer"
)
...
# ===== SAVE TO CSV =====
output_path = "/Group-project-Data-Science-in-Space/Workfolder Climate cases (Caspar)/combined_independent_variables.csv"
